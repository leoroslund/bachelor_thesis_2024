...
def main(setting: str, chargers: int = 2, excavator_bat: int = 0, charging_power=150):
    ...
    def plot_data(battery_levels_by_machine: dict, grid_power_list: list, active_machines: list, large: bool = False) -> None:    
        ...
        def plot_setup(title: str, xlabel: str, ylabel: str, x_ticks: np.ndarray, formatter) -> None:
            ...
            plt.ylim(bottom=0)
            #plt.axvline(x = 7200, color = 'r', alpha = 1, ls = ':', lw = 2)
            #plt.axvline(x = 18000, color = 'r', alpha = 1, ls = ':', lw = 2)
            #if chargers == 3 or chargers == 2:
                #plt.axvline(x = 9000, color = 'm', alpha = 1, ls = ':', lw = 2)
                #plt.axvline(x = 19800, color = 'm', alpha = 1, ls = ':', lw = 2)
            plt.tight_layout()
        ...
        plt.axhline(y=mean_power, c = "k", alpha = 0.5, ls = '--', lw = 3, label = "Medeleffekt")
    ...
    if setting.lower() == "med":
        excavator_conf: dict = {'battery_capacity': 264, 'operating_power': 105 / 3600,
        ...
        excavator_conf: dict = {'battery_capacity': 568, 'operating_power': 184 / 3600,
                            'high_power': 283, 'low_power': 36, 'high_time': 6, 'low_time': 4}
    ...
    print(f"Toppeffekt [kW]: {max(total_power):.0f}")
    print(f"Medeleffekt [kW]: {mean_power:.0f}")
    print(f"Totalt energibehov [kWh]: {mean_power*9:.0f}")
    print(f"Produktivitet: {1-(missed_hours/total_work_hours) :.1%}")
    plot_data(battery_levels, total_power, active_machines, large = large)

if __name__ == "__main__":
    main(setting="lar", chargers=2, excavator_bat=0, charging_power=350)
